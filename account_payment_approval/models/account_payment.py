...
class AccountPayment(models.Model):
    ...
    def _check_payment_approval(self):
        if self.state == "draft":
            ...
            # raise UserError(self.env.user.company_id)
            if first_approval :
                Usercompany_id=str(self.env.user.company_id.id)
                approval_company_id =str(self.env['ir.config_parameter'].sudo().get_param(
                            'account_payment_approval.approval_company_id'))
                if Usercompany_id == approval_company_id :
